            ...
            case V1Namespace ns when notification.EventType is WatchEventType.Added or WatchEventType.Modified:
                {
                    ...
                    foreach (var sourceNsName in AutoMirrorSourceCandidates().ToArray())
                    {
                        ...
                        {
                            var resource = await TryResourceGet(sourceNsName, cancellationToken);
        ...
        try
        {
            foreach (var nsName in candidates)
            {
                if (_propertiesCache.ContainsKey(nsName))
                    continue;
                var primed = await TryResourceGet(nsName, cancellationToken);
                if (primed is null)
                {
                ...
                await HandleUpsert(primed, cancellationToken);
            }
        }
        finally
        {
